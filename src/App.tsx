...
export default function App() {
  ...
  // BUG 2: Listen for RSC_DISCONNECT from the game iframe (WebSocket closed)
  useEffect(() => {
  ...
  // Step 1: Auth succeeded, we have provider + externalId
  ...
  // BUG 1: Existing user logged in — skip UsernamePicker, go straight to game
  ...
  // Step 2: Username picked, server returned RSC credentials
  const handleUsernameComplete = useCallback((rscUsername: string, rscPassword: string) => {
    setRscCredentials({ username: rscUsername, password: rscPassword });
    setAppState('loading');
  }, []);

  // Step 3: Game iframe reports login complete
