...
(function() {
  ...
  function makeCombatScript(npcIds, defaults) {
    ...
    return function() {
      ...
      // NPC corpses stay in b0.data during death animation (up to 90s!).
      // Use a 5-second combat timeout: if no kill detected in 5s, assume dead.
      if (scriptState.target >= 0) {
        var timeSinceAttack = Date.now() - (scriptState.lastAttackTime || 0);
        if (isTargetAlive(scriptState.target) && timeSinceAttack < 5000) {
          return 618;  // Still fighting — wait
        } else {
          // Target disappeared OR 5s timeout — treat as kill
          scriptState.killCount++;
          scriptState.target = -1;
          return 300;  // Fast transition to looting
        }
      }
