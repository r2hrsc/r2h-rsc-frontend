...
(function() {
  ...
  var F = {
    ...
    statFatigue: 'sp',      // current fatigue level (0-100)
  ...
  // Cooked food only (raw food can't be eaten)
  var FOOD = [335, 333, 350, 352, 355, 357, 359, 362, 364, 367, 370, 373, 718, 551, 553, 555, 546, 325, 326, 327, 328, 329, 330, 332, 334, 336, 257, 258, 259, 261, 262, 263, 210, 346, 709, 18, 228, 337, 132, 138, 142, 179];
  ...
  function atObject(worldX, worldY) {
    return sendRaw(242, 0, function(stream, Z, BO) {
  ...
  function sendRaw(opcode, type, payloadFn) {
    ...
    if (!mc || !mc.c || !mc.c.bX) return false;
    var stream = mc.c;
    var W = window.__r2h_W, Z = window.__r2h_Z, BO = window.__r2h_BO, Y = window.__r2h_Y;
    if (!W || !Z || !Y) return false;
  ...
  function findNpcs(npcTypeIds, maxDist) {
    ...
    if (npcTypeIds && npcTypeIds.length > 0) {
      ...
      log('NPC filter: [' + npcTypeIds.join(',') + '] → looking for bV values: ' + Object.keys(typeFilter).join(','));
    }
    ...
    for (var i = 0; i < mc.b0.data.length; i++) {
      ...
      if (typeFilter) {
        ...
        if (!typeFilter[npcType]) {
          if (skippedByFilter.length < 5) skippedByFilter.push(npcType);
          continue;
        }
      }
    ...
    if (typeFilter && results.length === 0 && skippedByFilter.length > 0) {
      log('No targets found matching [' + Object.keys(typeFilter) + '] — nearby types: ' + skippedByFilter.join(','));
    }
  ...
  function findObjectById(objectId) {
    ...
    if (!ox || !oy || !oid || !ox.data) return null;

    for (var i = 0; i < count; i++) {
      if ((oid.data[i] || 0) === objectId) {
        return {
          id: objectId,
          worldX: (ox.data[i] || 0) + (mc[F.regionX] || 0),
          worldY: (oy.data[i] || 0) + (mc[F.regionY] || 0),
          index: i,
        };
      }
    }
  ...
  function getFatigue() {
    var mc = getMC();
    return mc ? (mc[F.statFatigue] || 0) : 0;
  }

  function getIsSleeping() {
    var mc = getMC();
    return mc ? (mc[F.isSleeping] ? true : false) : false;
  }
  ...
  function setFightMode(mode) {
    // Payload177: opcode 231 = COMBAT_STYLE_CHANGED. putByte(style).
    // Also set the client-side field directly — the 177 protocol doesn't echo
    // combat style back to the client, so we must update the local UI ourselves.
    var mc = getMC();
    if (mc) mc[F.combatStyle] = mode;
  ...
  function startBot(scriptId, config) {
    ...
    scriptState = { phase: 'init', target: null, killCount: 0, lastAttack: 0, firstScan: true };
  ...
  function makeCombatScript(npcIds, defaults) {
    ...
    return function(runtimeCfg) {
      ...
      var cfg = {
        ...
        eatAtHp:         runtimeCfg.eatAtHp !== undefined ? runtimeCfg.eatAtHp : (defaults.eatAtHp || 10),
    ...
    return function() {
      ...
      // ══ 0. FIGHT MODE — match APOS: check + set every iteration ══
      // APOS line 659: if (c.getFightMode() != fightMode && targetLevel == -1) c.setFightMode(fightMode);
      if (cfg.fightMode >= 0 && cfg.fightMode <= 3 && cfg.targetLevel < 0) {
        var mcStyle = getMC();
        if (mcStyle && mcStyle[F.combatStyle] !== cfg.fightMode) {
          mcStyle[F.combatStyle] = cfg.fightMode;
      ...
      if (cfg.openDoors) {
        if (scriptState.target >= 0) {
          scriptState.stuckCount = (scriptState.stuckCount || 0) + 1;
        }

        if (scriptState.target >= 0 && scriptState.stuckCount > 2) {
          // Clear target first — server blocks gate interaction if inCombat/isBusy
          scriptState.target = -1;
          var px = getX(), py = getY();
          log('Stuck — opening gate near (' + px + ',' + py + ')');
          // Send atObject to ONE tile at a time, not all at once.
          // Start with the tile directly between player and nearest cow.
          var mcT = getMC();
          var gateX = -1, gateY = -1;
          if (mcT) {
            for (var si = 0; si < mcT.b0.data.length; si++) {
              var snpc = mcT.b0.data[si];
              if (!snpc || snpc.bV !== npcIds[0]) continue;
              var npcWX = Math.floor(snpc.F/128) + (mcT[F.regionX]||0);
              var npcWY = Math.floor(snpc.E/128) + (mcT[F.regionY]||0);
              // Gate is between player and NPC — step toward NPC
              if (npcWX < px) gateX = px - 1;
              else if (npcWX > px) gateX = px + 1;
              else gateX = px;
              if (npcWY < py) gateY = py - 1;
              else if (npcWY > py) gateY = py + 1;
              else gateY = py;
              break;
            }
          }
          if (gateX < 0) { gateX = px; gateY = py; }
          // Try the step-toward-NPC tile first, then expand outward
          atObject(gateX, gateY);
          // Also try a few more tiles in a line toward the NPC
          atObject(gateX + (gateX - px), gateY + (gateY - py));
          atObject(px, py);
      ...
      if (currentHp <= cfg.eatAtHp && currentHp < maxHp) {
        for (var f = 0; f < FOOD.length; f++) {
          var foodSlot = getInventoryIndex(FOOD[f]);
          if (foodSlot >= 0) {
            log('Eating: HP ' + currentHp + '/' + maxHp);
            useItem(foodSlot);
            return 800;
          }
        }
      ...
      // APOS: the blocking c.attackNpc() returns when combat ends.
      // We can't block, so we poll: if target is set and still alive, wait.
      if (scriptState.target >= 0) {
        if (isTargetAlive(scriptState.target)) {
          // Target still alive — WAIT. Do not send any packets.
          // Sending anything triggers server resetAll() which cancels combat.
          return 618;  // APOS: c.sleep(618) per iteration
        } else {
          // Target died!
          scriptState.killCount++;
          log('KILL #' + scriptState.killCount);
          scriptState.target = -1;
          // Small delay to let loot appear on ground
          return 618;
        }
      }

      // ══ 5. LOOT + BURY (only when not in combat) ══
      // APOS: for (int lootId : lootTable) { pickup; sleep(618); bury; }
          ...
          if (cfg.buryBones) {
            for (var bi = 0; bi < BONES.length; bi++) {
              ...
              if (boneSlot >= 0) {
                ...
                return 640;  // APOS: c.sleep(640) after bury
              }
            }
          }
          return 618;  // APOS: c.sleep(618) after pickup
        }
      }

      // ══ 6. FIND + ATTACK NEAREST NPC ══
      // APOS line 690: ORSCharacter npc = c.getNearestNpcByIds(npcIds, false);
      // APOS line 698: c.attackNpc(npc.serverIndex); c.sleep(1000);
      ...
      if (targets.length > 0) {
        ...
        log('Attacking NPC idx=' + target.serverIndex + ' type=' + target.npcType + ' dist=' + target.dist);
        attackNpc(target.serverIndex);
        scriptState.target = target.serverIndex;
        // APOS: c.sleep(1000) — give server 1s to start walking to NPC
        return 1000;
      }
  ...
  function isInBank() {
    var mc = getMC();
    if (!mc) return false;
    if (_bankDialogField !== null) {
      try { return !!mc[_bankDialogField]; } catch(e) {}
    }
    // Probe: scan mc fields for a boolean that's true while bank is open
    // Known field names from APOS: showDialogBank
    // TeaVM obfuscated — try common patterns
    var candidates = ['cI', 'cJ', 'cK', 'dv', 'dw', 'dx', 'eE', 'hT', 'hU', 'hV'];
    for (var i = 0; i < candidates.length; i++) {
      try {
        var v = mc[candidates[i]];
        if (v === true || v === 1) {
          _bankDialogField = candidates[i];
          return true;
        }
      } catch(e) {}
    }
  ...
  function setFightMode(mode) {
    sendRaw(231, 700, function(stream, Z, BO) {
      BO(stream, mode);
    });
    return true;
  }
  ...
  // ─── Fatigue ───
  // APOS uses reflector field "fatigueSleeping" on the mudclient.
  // TeaVM: scan for the field that holds a value 0-100 representing fatigue %.
  // In the original Java: fatigue * 100 / 750 — so the raw field is 0-750.
  // We'll probe for a field in range [0, 750] that increases over time.
  var _fatigueField = null;
  var _fatigueLastVal = -1;
  function getFatigue() {
    var mc = getMC();
    if (!mc) return 0;
    if (_fatigueField !== null) {
      try {
        var v = mc[_fatigueField];
        if (typeof v === 'number') {
          _fatigueLastVal = v;
          return Math.floor(v * 100 / 750);
        }
      } catch(e) {}
    }
    // Fallback: return 0 (no fatigue system detected)
    return 0;
  }
  ...
  // ─── Path Walking ───
  // Walks a sequence of tiles. Returns true when the final tile is reached.
  // In tick-based architecture, call walkTo(nextTile) each tick until close.
  var _pathState = null;
  function walkPath(path) {
    if (!path || path.length === 0) return true;
    if (!_pathState || _pathState.path !== path) {
      _pathState = { path: path, idx: 0 };
    }
    var target = path[_pathState.idx];
    var px = getX(), py = getY();
    var dist = Math.abs(px - target.x) + Math.abs(py - target.y);
    if (dist <= 2) {
      _pathState.idx++;
      if (_pathState.idx >= path.length) {
        _pathState = null;
        return true; // Path complete
      }
      target = path[_pathState.idx];
    }
    walkTo(target.x, target.y);
    return false;
  }
  function walkPathReverse(path) {
    if (!path || path.length === 0) return true;
    var reversed = path.slice().reverse();
    return walkPath(reversed);
  }
  function resetPath() {
    _pathState = null;
  }
